...
const navigator = {
    ...
    cookieEnabled: false,       // Node 没有 cookie 概念
    geolocation: false,         // Node 默认没有 geolocation
    javaEnabled: () => false,   // Node 不支持 java
    language: process.env.LANG ? process.env.LANG.split('_')[0] : 'en',
    msMaxTouchPoints: 0,
    onLine: true,               // 假设 Node 在线
    platform: process.platform,
    plugins: [],                // Node 没有插件
    preference: null,
    product: 'nodejs',
    systemLanguage: process.env.LANG || null,
    userAgent: `node/${process.version}`,
    userLanguage: process.env.LANG || null,
    msLaunchUri: null,
    taintEnabled: false,
...
function md5(I, F) {
    function O(C, U) {
        const Z = navigator.appCodeName.toLowerCase();
        const T = navigator.appName.toLowerCase();
        const j = navigator.appVersion.toLowerCase();
        const k0 = navigator.cookieEnabled;
        const k1 = navigator.geolocation;
        const k2 = navigator.javaEnabled();
        const k3 = navigator.language.toLowerCase();
        const k4 = navigator.msMaxTouchPoints;
        const k5 = navigator.onLine;
        const k6 = navigator.platform;
        const k7 = navigator.plugins;
        const k8 = navigator.preference;
        const k9 = navigator.product;
        const kk = navigator.systemLanguage;
        const kI = navigator.userAgent;
        const kx = navigator.userLanguage;
        const kF = navigator.msLaunchUri;
        const ka = navigator.taintEnabled;
        const kO = navigator.hasOwnProperty;
        const kc = document.hasOwnProperty('ontouchstart');
        return C << U | C >>> 0x20 - U;
    }
    ...
    function u(C) {
        var U, Z = C['length'], T = Z + 0x8, j = (T - T % 0x40) / 0x40, k0 = (j + 0x1) * 0x10, k1 = Array(k0 - 0x1),
            k2 = 0x0, k3 = 0x0;
        while (k3 < Z) {
            U = (k3 - k3 % 0x4) / 0x4,
                k2 = k3 % 0x4 * 0x8,
    ...
    ;var J = Array(), H, h, n, t, N, D, v, M, g, y = 0x7, E = 0xc, L = 0x11, K = 0x16, B = 0x5, X = 0x9, A = 0xe,
        q = 0x14, V = 0x4, o = 0xb, R = 0x10, G = 0x17, Y = 0x6, e = 0xa, z = 0xf, f = 0x15;
    I = p(I),
        J = u(I),
        D = 0x67452301,
        v = 0xefcdab89,
